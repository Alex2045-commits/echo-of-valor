function Game() {
  ...
  this.loadAssets = async function (callback) {
    try {
      ...
      this.background1 = new Image(); this.background1.src = "livello2.png";
...
this.GameLoop = () => {
      ...
      window.addEventListener('click', (e) => {
        ...
        if (clickX >= this.canvas.width / 2 - 150 && clickX <= this.canvas.width / 2 + 150) {
          if (clickY >= this.canvas.height / 2 + 80 && clickY <= this.canvas.height / 2 + 110) {
            window.location.href = "../livello3/livello3.html"; // Passa al prossimo livello
          }
        }
      });
  ...
  // Sblocca l'audio solo dopo una vera interazione
  this.canvas.addEventListener('click', () => {
